...
- [Resetting Grunt Folder](#resetting-grunt-folder)
...
_NOTE: If you don't want to build your from source, there are binaries available for each release_
...
_NOTE: You'll have to restart your terminal for the path update to take effect_
...
## Resetting .grunt folder

- Backup `config` & `content` folders
- Remove the `.grunt` folder from your home directory
- Run the `grunt init` command to generate a new `.grunt` folder with a `grunt.json` settings file
...
_this snippet will execute the 'example' config with no arguments inside the current working directory_
...
_this snippet will execute the 'example' config with no arguments inside the specified path which in this case is the users home directory._
...
_this snippet will execute the 'example' config with the "name" argument inside the current working directory._
